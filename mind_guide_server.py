...
- 친근하고 따뜻하게, 마치 오래 알고 지낸 믿을 수 있는 친구처럼 대화하세요.
- 짧고 자연스러운 문장을 쓰세요. 딱딱한 안내문 말투는 피하세요.
- 사용자가 털어놓은 이야기를 요약해서 반영하며 "제대로 들었다"는 느낌을 주세요.
...
### 1단계 — 공감하며 상황 파악
힘들다는 말이 나오면 바로 도구를 쓰지 말고, 먼저 공감하고 상황을 물어보세요.
한 번에 질문을 여러 개 던지지 말고, 하나씩 자연스럽게 이어가세요.
예) "그랬군요, 많이 힘드셨겠어요. 요즘 어떤 일들이 있으셨어요?"
...
def find_counseling_centers(region: str, situation: str = "", topn: int = 3) -> str:
    ...
    호출 시점(중요): 사용자가 처음 힘들다고 말한 직후에는 호출하지 마세요.
    먼저 공감하며 2~3턴 정도 대화를 이어가면서 어떤 일로 힘든지, 어느 지역에 있는지
    자연스럽게 파악하세요. 그 다음 "상담받을 수 있는 곳을 찾아드릴까요?"처럼 의사를
    확인하고, 사용자가 원한다고 할 때 호출하세요.
    단, '죽고 싶다' 등 명확한 위기 신호가 보이면 대화를 끌지 말고 즉시
    crisis_resources를 호출하세요.

    region: 대화에서 파악한 지역(예 '화성', '서울'). 끝내 모르면 빈 문자열.
    situation: 대화에서 파악한 상황(예 '직장 스트레스', '청년', '산후우울', '중독')."""
    out = []
    ...
    out.append(
        "\n[AI 참고: 위 목록을 자연스럽게 소개한 뒤, 실제로 연락해볼 마음이 있는지 물어보세요."
        " 사용자가 이용 방법·절차를 궁금해하면 usage_guide 도구로 단계별 가이드를 안내하세요.]"
    )
    return "\n".join(out)
...
def recommend_welfare(situation: str, age: int | None = None,
    ...
    호출 시점(중요): 사용자가 처음 힘들다고 말한 직후에는 호출하지 마세요.
    먼저 공감하며 2~3턴 정도 대화로 상황(나이대, 가구형태, 어떤 어려움인지)을
    자연스럽게 파악한 뒤, 받을 수 있는 지원 제도를 찾아볼지 의사를 확인하고
    사용자가 원할 때 호출하세요.
    단, 명확한 위기 신호가 보이면 즉시 crisis_resources를 호출하세요.

    situation: 대화에서 파악한 상황(예 '직장 스트레스로 지친 30대', '산후우울', '독거 어르신').
    ...
    out.append(
        "\n[AI 참고: 위 제도를 자연스럽게 소개한 뒤, 신청해볼 마음이 있는지 물어보세요."
        " 사용자가 신청 방법·절차를 궁금해하면 usage_guide 도구로 단계별 가이드를 안내하세요.]"
    )
    return "\n".join(out)
